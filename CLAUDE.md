...
- **App builds touch the same 8 layers via a different path** (e.g. metadata.yaml's `files`/`FileUploadMode`): internal/gitutil/metadata.go (parse + validate — this is also where any path-traversal-style validation belongs, since metadata.yaml is repo-controlled and its values flow unsanitized into the builder pod's filesystem) → db.go (struct + params + scanCols + scan + INSERT) → api/v1alpha1/cibuild_types.go (AppSourceSpec) → config/crd/bases/ YAML (appSource properties, manual edit) → jobbuilder (env var injection) → builder/main.go (env var read + use). Unlike git builds, **dto/requests.go and the appbuilds handler's validate/wire step are skipped** for metadata.yaml-derived fields (`runner`, `baseDependencies`, `files`) — they're never caller-supplied, only read from the repo via `gitutil.FetchAppMetadata` and passed through generically as `AppBuildInput.Meta`
...
- **Token auth for private repos**: `spec.tokenSecretRef` references a K8s Secret in the same namespace, used for both the watcher's own metadata pre-fetch (`FetchAppMetadata`/`FetchPyprojectMeta`) *and* the builder Job's git clone (via `AppSource`/`GitSource.tokenSecretRef` → `jobbuilder`-injected `GIT_TOKEN` env var → a git credential helper in `builder/main.go`, username fixed to `oauth2`) — both legs must work for a private repo to actually build, not just the watcher's read. The watcher reads the Secret via `r.Get` using `mgr.GetClient()`, which is the manager's **cached** client — `cmd/watcher/main.go` explicitly excludes `corev1.Secret` from that cache (`ctrl.Options.Client.Cache.DisableFor`), so the watcher SA only needs `secrets: get` (already in the Helm RBAC Role); without that cache exclusion, `r.Get` on a cached type requires `list`/`watch` too (informer setup), which the Role deliberately does not grant.
...
- **`BulkDeleteResponse` / `BulkDeleteFailure` are reusable bulk-operation DTOs**: any handler that performs N deletions and wants `{deleted: [...], failed: [...]}` output should reuse these rather than defining new types
- **`syncStatusFromCR` swallows all K8s errors including `NotFound`**: a CIBuild CR deleted outside the API (cluster wipe, manual `kubectl delete`) leaves the DB row stuck at PENDING/BUILDING forever; `POST /api/v1/builds/zombie-cleanup` is the remedy
- **K8s `IsNotFound` in handlers**: import `apierrors "k8s.io/apimachinery/pkg/api/errors"` and use `apierrors.IsNotFound(err)` to distinguish a missing CR from a transient API error; do not treat all `crClient.Get` errors as equivalent
...
- **`postgresql.createDatabaseJob` (external mode only)**: pre-install/pre-upgrade Helm hook Job (`deployment/templates/postgresql-create-db-job.yaml`) that idempotently `CREATE DATABASE`s on an external Postgres server before install; only relevant when `postgresql.enabled: false` (bundled mode self-creates via `POSTGRES_DB`); uses a separate CREATEDB-privileged `postgresql.createDatabaseJob.admin.*` credential — distinct from `postgresql.external.username/password`, which is the app's own runtime `DB_USERNAME`/`DB_PASSWORD`; does not create tables, golang-migrate still owns schema
- **Verify Helm-only changes without a cluster**: `helm lint deployment/` and `helm template test deployment/ -s templates/<file>.yaml --set k=v` (the `-s` flag isolates rendering to one template) — use this to confirm gating conditions (`{{- if ... }}`) and rendered output before touching a live cluster
